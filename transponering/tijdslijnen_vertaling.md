...
Als er geen opvolgende versie bestaat: dan is er toch wel een `eindRegistratie`. Zie vertaal specificatie van beeindigde objecten, die een nieuw opvolgende objectversie creëert (met een eindstatus). Het `tijdstipRegistratie` van dat _opvolgende_ voorkomen is dat de `eindRegistratie` van _deze_ versie. 
...
ALS een versie uit de BGT een gevulde `objectEindtijd` (8.1.2) heeft dan is deze versie de _laatste versie_ uit de (LV)BGT. 
